...
RC TableMeta::init(int32_t table_id, const char *name, int field_num, const AttrInfoSqlNode attributes[])
{
  ...
  if (trx_fields != nullptr) {
    ...
    for (size_t i = 0; i < trx_fields->size(); i++) {
      ...
      fields_[i] = FieldMeta(field_meta.name(), field_meta.type(), field_offset, field_meta.len(), false /*nullable*/, false/*visible*/);
  ...
  for (int i = 0; i < field_num; i++) {
    ...
    rc = fields_[i + trx_field_num].init(attr_info.name.c_str(), 
            attr_info.type, field_offset, attr_info.length, attr_info.can_null, true/*visible*/);
    if (rc != RC::SUCCESS) {
...
{
  for (const IndexMeta &index : indexes_) {
    if (0 == strcmp(index.field_metas()[0].name(), field)) {
      return &index;
    }
  }
  return nullptr;
