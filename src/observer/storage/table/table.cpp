...
RC Table::create_index(Trx *trx, const std::vector<const FieldMeta*>& field_metas, const char *index_name, bool is_unique)
{
  ...
  if (common::is_blank(index_name) || field_metas.empty()){
    LOG_INFO("Invalid input arguments attribute_name( orindex_name) is blank or no attribute provided: length %d"
            ,field_metas.size());
    return RC::INVALID_ARGUMENT;
  }
  //std::vector<IndexMeta> new_index_metas;
  ...
  rc = index->create(index_file.c_str(), new_index_meta, new_index_meta.field_metas());
    ...
    for(int j = 0; j < field_num;j++){
      ...
      if(strcmp(field_name,update_field_meta->name())==0){
        ...
        if(field_type==update_value_type);
          ...
          LOG_DEBUG("Type Convert Happens");
