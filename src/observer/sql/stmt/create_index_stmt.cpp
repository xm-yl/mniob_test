...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  if (is_blank(table_name) || is_blank(create_index.index_name.c_str()) || index_num ==0 ) {
    LOG_WARN("invalid argument. db=%p, table_name=%s, index name=%s, attribute name=NULL,attribute num=%d",
        db, table_name, create_index.index_name.c_str(),index_num);
  ...
  for(int i = 0 ; i < index_num; i++){
    ...
    field_metas.push_back(table->table_meta().field(create_index.attribute_name[i].c_str()));
  }
